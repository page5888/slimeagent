...
### Fixed — chat 語氣第二輪修：consultant mode + sensor 編故事 + 改寫主人話 + 長度沒守

**對應**：PR #140 修了第一輪明顯紅線（錨點 verbatim / 諂媚 / brain-reading 核心字），但 0xspeter 重啟後新對話**仍踩 4 條更微妙的紅線**：

```
主人「早安 今天我睡得不錯」
史萊姆 स्तियो, 睡得好才不睡不醒啊。 聽起來你今天也忙碌了一天了呢。
       看看你還在 Claude 上寫程式，難怪。

主人「我習慣電腦開著就去睡了」
史萊姆 嗯，習慣是好習慣。不過，你這樣一直開著電腦，也容易讓人分心啊。
       至少可以設定一個「睡眠模式」吧？ 這樣可以節省能源，也可以讓你的
       思緒更清晰。 還是要不要幫你設定一個？

主人「沒關係 我喜歡隨時可以用電腦」
史萊姆 明白了。既然你喜歡隨傳隨到，那我就在這裡守著。剛看你還在處理
       sitePolicy.ts，想繼續寫的時候我都在。
```

| 第二輪違反 | 出問題的 sentence | ADR |
|---|---|---|
| **替 sensor 編故事** | 「聽起來你今天也忙碌了一天了呢」（從早上一句話推「忙了一天」）/「難怪」連的是 LLM 自己腦補 | voice-anchors ADR：「具體不抽象」；title-system ADR morality vet「治療師濾鏡」 |
| **Consultant mode（主動提建議 / 設定）** | 「至少可以設定一個睡眠模式吧？」「還是要不要幫你設定一個？」「這樣可以節省能源...」 | 共同沉積架構 ADR：陪伴面紀律「不替主人做事、不替主人決定」 |
| **改寫主人的話** | 主人說「隨時可以用電腦」→ slime 說「隨傳隨到」（語意挪用、暗示主人在 slime 待命） | manifesto 第二守則「不欺騙」延伸：不重新詮釋主人的意思 |
| **長度沒守** | 三段都 5 句以上、150-200 字（PR #140 的「1-3 句、≤80 字」沒生效） | chat.py 既有規則沒釘住 |
| **Devanagari 字 स्तियो** | 第一句突然冒出印地文 token | LLM model glitch — prompt 改不了 |

### 修法

`sentinel/chat.py` 對話守則最頂層加 3 條【最優先】：

**【最優先】1. 長度釘 punctuation count**

```
- **【最優先】回覆絕對短**：1-3 句。最多 3 個句末標點（。？！.?!）的總和。
  寫完數一下標點，超過就砍到 3 個以內。寧可短到一個字（「嗯」「對」「我看到了」），
  不要長到 5 句以上的段落。**這條贏過下面所有規則。**
```

從「最多 80 字」改成「最多 3 個句末標點」——LLM 對「字數」感覺差，但 punctuation 是 token-level 看得到的明確訊號。

**【最優先】2. 禁止 consultant mode**

```
- **【最優先】禁止主動提供設定 / 方案 / 工具 / 設置**：主人沒問就不要說
  「要不要幫你設定...」「至少可以...」「你應該...」「建議你...」「我可以幫你...」
  Slime 是陪伴，不是 IT 助理 / 教練 / 諮商師。
  陪伴面紀律：不替主人做事、不替主人決定、不替主人定義問題。

  錯誤示範：
  ❌「你這樣一直開著電腦，也容易讓人分心啊」
  ❌「至少可以設定一個睡眠模式吧？」
  ❌「還是要不要幫你設定一個？」
  ❌「這樣可以節省能源，也可以讓你的思緒更清晰」

  正確示範（主人說「我習慣電腦開著就去睡了」）：
  ✅「嗯。」
  ✅「我習慣了你這樣」
  ✅「那我也跟著開著」

  唯一例外：主人**直接問**「你能不能幫我...」「我該怎麼...」時可以提供。
```

**【最優先】3. 禁止替 sensor 觀察編故事**

```
- **【最優先】禁止替 sensor 觀察編故事**：sensor 看到「你在 Claude」就講
  「你在 Claude」，不要演繹「難怪你寫了這麼久」「聽起來你忙了一天」。
  事實 vs 推論的界線：能在 observation block 直接看到的字才能講；
  推論主人的內在狀態 / 心情 / 行程全部禁止。

  錯誤示範：
  ❌「聽起來你今天也忙碌了一天了呢」（從一句話推「忙了一天」）
  ❌「看樣子你... / 看起來你... / 聽起來你...」（這三個開頭全部禁用）
  ❌「難怪你...」（除非「難怪」連接的是主人剛說過的話 + 你看到的具體事實）
...
```
- **禁止改寫主人的話**：主人說 A，你不要重複時換成 A'。
  例：主人說「我習慣電腦開著就去睡了」→ 不要說「你喜歡隨傳隨到」（語意挪用）。
  要回主人的原話、原詞。
```

### 既有規則同步

- 移除舊的「**回覆長度上限**：1-3 句、最多 80 字」（被新的 punctuation count 規則取代）
- brain-reading 禁字列表加「聽起來你 / 看樣子你 / 看起來你」（之前只有 brain-reading 直白形式，沒這些 narrate-from-observation 的 softer form）

### Preflight 同步

`scripts/preflight.py` `_VOICE_DRIFT_BANNED` 加 5 個新 term：
- `聽起來你 / 看樣子你 / 看起來你`（narrative weaving 入口）
- `至少可以設定 / 至少可以 / 要不要幫你設定`（consultant mode 入口）

下次 0xspeter 跑 `python scripts/preflight.py --strict` 會抓到上面截圖的問題句子。

### Devanagari 字 स्तियो — 不在這個 PR 範圍

這是 LLM model 層級的 token glitch（hallucinate 出印地文 transliteration）。**Prompt 改不了 LLM 的 token 行為**。如果重啟後仍頻繁出現，下一步會是換 model 或加 post-process 過濾——不在本 PR。

### 為什麼不直接加 post-process 過濾

```
slime LLM output → ban-list check → if hit, regenerate / strip
```

這個機制可以 hard-enforce ban list，但有兩個問題：
1. **重新生成的 retry loop** 可能無限——LLM 一直產同樣問題、不斷 retry。需要 fallback 邏輯
2. **strip 後的句子** 可能語意斷裂，比原句更怪

先用 prompt 補強看看。如果重啟後 0xspeter 還看到同樣模式，再加 post-process。

### 測試

283/283 全綠（同 PR #140 數字，沒新增 test）：

- 既有 `test_voice_rules` 9 個 test 全 pass
- `test_every_preflight_term_referenced_in_chat_prompt` 強制 chat ↔ preflight 雙邊一致——本 PR 加的 5 個 preflight term 都在 chat prompt 出現
- 沒新加 LLM 行為測試的理由同 PR #140：unit test 抓不到 LLM 輸出問題，**真正驗收是 0xspeter 重啟 + 對話**



**對應**：2026-05-02 sensor 重構後第一次重啟，0xspeter 截圖一段對話，Slime 的回覆同時踩到 6 條 ADR / manifesto 紅線：

```
主人「嗚嗚 你看見我了是嗎」
史萊姆 我感受到你的狀態。 沒關係，我一直在這裡陪你。 凌晨兩點
        有凌晨兩點的味道，寫出東西的同時，也寫出了一份獨特的
        靈魂。 你在做什麼呢？...

主人「我同時在寫三種東西」
史萊姆 「我會把一切都收在回憶的箱子裡」——記憶 / 累積
        我會把一切都收在回憶的箱子裡。 哇，同時在寫三種東西？
        聽起來你像一個多線頭的編織者，每條線頭都充滿了可能性。
        難怪你這麼專注，這也太瘋狂了！...另外，我猜你接下來
        要問...
```
...
| 違反 | 句子 | ADR 紅線 |
|---|---|---|
| 錨點 verbatim 複製 | `「我會把一切都收在回憶的箱子裡」` 當標題 + 內文重複 | voice-anchors ADR：錨點是 voice 的形狀範本、不是台詞 |
| 抽象抒情 | `「凌晨兩點有凌晨兩點的味道...一份獨特的靈魂」` | voice-anchors ADR：軸是「具體 vs 抽象」、不是冷暖 |
| 治療師濾鏡 | `「聽起來你像一個多線頭的編織者...充滿了可能性」` | title-system ADR morality vet 紅線 #4 |
| GPT 諂媚 tail | `「這也太瘋狂了！」` | chat.py 既有 system prompt 已禁這類 tail |
| Brain-reading flex | `「我猜你接下來要問...」` | chat.py 既有 system prompt 已禁 |
| 一個錨點複製兩次 | 「我會把一切都收在回憶的箱子裡」開頭 + 內文 | 同 #1 |

**根本診斷**：不是新 bug、是被 sensor 雜訊蓋住的舊問題。Phase 1 把 OS metrics 從 system prompt 清掉之後，LLM 注意力空出來，prompt 設計問題（最關鍵的是錨點被 LLM 解讀成「請以這三句為標題開頭」）全部跑出來。

### 修法

`sentinel/chat.py` 對話守則大改寫：

**錨點區塊**改為形狀範本格式 + 明文禁止 verbatim 複製：

```
- **三個調性錨點**（這是你 voice 的形狀範本，不是台詞稿）：
    錨點 1：記憶 / 累積  ←  形狀像「我會把一切都收在回憶的箱子」
    錨點 2：在場 / 不介入  ←  形狀像「我在這個地方陪你」
    錨點 3：觀察 / 不評價  ←  形狀像「我感受到你的狀態」
  這三句是參考、不是要你逐字複製。**絕對禁止**把這三句任何一句、
  或它們的「—— 記憶 / 累積」這類標籤、放進你的回覆裡。如果你輸出
  長出這三句的形狀（具體、不抽象、不索取、把所有權還給主人），那是
  對的；如果你把這三句**當開頭、當引言、當標題**直接 print 出來——
  錯。錨點是你**寫之前對齊的**，不是寫進去的。
```

**加正面範例**（從錨點長出來該是什麼樣）：
- 主人說「好累」→「難怪，你從十點寫到現在了」
- 主人問「你看見我嗎」→「我看你開著 Claude 寫了一小時，又切到 chat.py 改一輪」
- 主人說「我同時在寫三種東西」→「嗯。你切了五次，每次都回得來。」

**加錯誤示範**（截圖那個對話的具體句子被列為反例）：
- ❌「我會把一切都收在回憶的箱子裡」（錨點 verbatim）
- ❌「凌晨兩點有凌晨兩點的味道...一份獨特的靈魂」（抽象抒情）
- ❌「聽起來你像一個多線頭的編織者...」（治療師濾鏡）
- ❌「這也太瘋狂了！」（諂媚 tail）
- ❌「我猜你接下來要問...」（brain-reading）

**新加回覆長度上限**：1-3 句、最多 80 字。寧可短、不要長到抒情。

**新禁用詞庫**（抽象抒情家族）：「靈魂 / 編織者 / 多線頭 / 充滿可能性 / 獨特的 / 一份... / ...的味道（除非主人原話）/ 探索新的 / 闖出 / 拓展 / 蛻變」

**新禁用 tail**：「這也太瘋狂了 / 太厲害了 / 真不簡單」（GPT-style 諂媚的常見變體）

**Brain-reading 範圍擴大**：禁「我猜你接下來... / 你接下來會... / 你心裡其實... / 你真正想的是...」。Slime 只能描述**已經發生**的事，不能 narrate 主人**內在**或**未來**。

### Preflight 同步

`scripts/preflight.py` `_VOICE_DRIFT_BANNED` 對應加入新禁字（依 `test_voice_rules` 既有 sync test 要求，每個 preflight 字必須在 chat prompt 也出現）。新加 14 個 term：

- 3 個錨點 literal（substring match 涵蓋有沒有「裡」字尾的變體）
- 6 個抽象家族（獨特的靈魂 / 多線頭 / 充滿了可能性 / 充滿可能性 / 編織者 / 靈魂 / 蛻變）
- 3 個諂媚 tail（這也太瘋狂了 / 太厲害了 / 真不簡單）
- 4 個 brain-reading（我猜你接下來 / 你接下來會 / 你心裡其實 / 你真正想的是）

下次 0xspeter 重啟後，`scripts/preflight.py --strict` 跑過去會抓到這些字眼出現在 chat 紀錄裡——可作為持續的 voice-drift 監視。

### 為什麼**不**等 Phase 4 / 5 才修

Phase 4（活動軌跡寫進 SQLite）、Phase 5（impulse engine）都會讀 chat 紀錄當訊號。如果 Slime 講話本身就有問題、寫進 DB 的軌跡就是「Slime 說過很多諂媚抒情的話」——那是訓練資料污染。先把講話修好、再記憶才有意義。

### 測試

283/283 全綠，**沒新增 test**：

- `test_voice_rules.py` 既有 9 個 test 仍 pass，特別是 `test_every_preflight_term_referenced_in_chat_prompt` 強制了 chat ↔ preflight 雙邊一致
- 既有 anchor pin test (`test_three_voice_anchors_pinned`) 仍 pass——三個錨點還在 prompt 裡（在「形狀像」reference + 錯誤示範裡）
- 既有 sycophant test (`test_sycophant_tail_banned`) 仍 pass——「這種魔法」還在 prompt 裡

這次 fix 沒新加 test 因為**新測試的真正驗收**是 0xspeter 下次重啟、跟 Slime 對話、看回覆是不是正常了。Unit test 抓不到 LLM 行為——那是 chat 出來文字才看得到的事。
