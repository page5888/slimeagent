...
要標記，回（letter_to_master 跟 master_phrase 都是可選的，預設應該留空）：
  {{"mark": true, "headline": "（≤60 字，第一人稱，像在自言自語，不是在對主人說話）", "detail": "（≤120 字，多寫一點為什麼這個瞬間值得記下來）", "letter_to_master": "（可選，≤120 字，這是你直接寫給主人看的一句話，不是給自己的內心獨白；他下次滑時間軸點到這個節點時會看到。沒有想說的就不要寫，不要為了寫而寫）", "master_phrase": "（可選，≤80 字，這是主人最近說過、你覺得值得記下來當作「你跟主人之間專屬語彙」的一句**逐字原文**。必須從上面「主人最近說過的話」那個區塊裡挑，**不准改寫、不准翻譯、不准捏造**。挑進來的話之後在聊天時你可能會自然回引——所以挑的標準是：這句話有沒有特別的形狀、特別的比喻、或主人專屬的口頭禪。一般的提問跟回應**不要挑**。沒有特別值得記的就留空字串）"}}
...
判斷原則：
  ...
  - **master_phrase 比 letter 還更稀有**：絕大多數的日子主人說的話都是普通的提問跟回應，**不該挑**。一個月可能只挑得到 1-2 個真的有形狀、值得放進「Slime 之語」字典的句子。寧可留空，也不要為了塞而塞——如果挑了一句沒形狀的話進來，下次你引用它的時候會很怪。"""
...
def _build_signals(now: float) -> Optional[dict]:
    ...
    # Recent master words — verbatim quotes from the last few user
    # messages. Source for the optional `master_phrase` field in the
    # output schema (ADR 2026-04-30 共同沉積 mechanism 3): without a
    # real verbatim source, the LLM has no honest way to populate
    # master_phrase and would hallucinate. We pass actual quotes so
    # slime can pick one only if a real quote is worth keeping.
    master_words = _load_recent_master_words(now)
...
# Cap the chat-history feed at 10 lines and 80 chars each — small
# enough to stay cheap, large enough to give the LLM real context to
# pick a phrase from.
_MASTER_WORDS_LIMIT = 10
_MASTER_WORDS_PER_LINE = 80
# Only consider master messages from the last 2 days. Older quotes are
# already candidates for retrieval through the normal moments path; the
# point of feeding fresh ones is to capture phrases said *recently*
# that haven't crystallised into a moment yet.
_MASTER_WORDS_LOOKBACK_SECONDS = 2 * 86400


def _load_recent_master_words(now: float) -> list[str]:
    """Read up to N most-recent verbatim user messages from chat log.

    Returns a list of trimmed strings (newest last). Empty list if the
    chat log is missing, unreadable, or contains no recent master
    messages — caller treats that as "no source available, skip
    master_phrase capture this round".

    Defensive: any IO/decode error returns []. Logging is INFO-level
    so missing file is visible during debugging without flooding logs.
    """
    import json as _json
    from pathlib import Path

    chat_path = Path.home() / ".hermes" / "sentinel_chats.jsonl"
    if not chat_path.exists():
        return []

    cutoff = now - _MASTER_WORDS_LOOKBACK_SECONDS
    out: list[str] = []
    try:
        with chat_path.open("r", encoding="utf-8") as f:
            # Read full file then walk backward — chat log is small
            # enough (KB-MB range) that this is fine, and walking
            # backward lets us stop early once we hit the cutoff.
            lines = f.readlines()
    except OSError as e:
        log.info("co-reference: could not read chat log: %s", e)
        return []
    ...
    for raw in reversed(lines):
        ...
        if not text:
            continue
        if len(text) > _MASTER_WORDS_PER_LINE:
            text = text[:_MASTER_WORDS_PER_LINE] + "…"
        out.append(text)
        if len(out) >= _MASTER_WORDS_LIMIT:
            break
...
def _format_user_prompt(signals: dict) -> str:
    ...
    master_words = signals.get("master_words") or []
    if master_words:
        words_block = "\n".join(f"  - 「{w}」" for w in master_words)
        words_section = (
            f"\n主人最近兩天說過的話（這是你決定 master_phrase 時唯一能引用的來源——如果這幾句裡沒有特別值得當作「Slime 之語」記下來的，master_phrase 就留空，**不要從這幾句之外捏造**）：\n"
            f"{words_block}\n"
        )
    else:
        words_section = (
            "\n（最近沒有讀到主人的任何訊息——master_phrase 必須留空，這一輪沒有來源可引用。）\n"
        )
    return (
        f"現在的狀態：\n"
...
def record_emergent_moment_if_due() -> bool:
    ...
    # master_phrase is optional and CAPPED MORE TIGHTLY (80) than the
    # other text fields — co-reference anchors only work if they're
    # short distinctive snippets ("水底嗎?"), not whole sentences.
    phrase = str(obj.get("master_phrase", "") or "").strip()[:80]
    # Anti-hallucination guard: master_phrase must be a substring of
    # something we actually fed the LLM, otherwise it's made up.
    # Compare against the master_words signals — if no match, drop the
    # phrase silently (don't drop the whole mark; headline/detail/
    # letter can still be valid).
    if phrase and signals.get("master_words"):
        sources = [w.rstrip("…") for w in signals["master_words"]]
        if not any(phrase in src or src in phrase for src in sources):
            log.info(
                "emergent self-mark: master_phrase %r not found in source "
                "words, dropping phrase but keeping mark", phrase,
            )
            phrase = ""
    elif phrase and not signals.get("master_words"):
        # No source was given to the LLM, so any phrase it produced is
        # hallucinated. Drop unconditionally.
        log.info("emergent self-mark: master_phrase produced with no source, dropping")
        phrase = ""
    ...
    # detail, the letter, AND the master_phrase echo. The letter is the
    # highest-stakes channel (rendered prominently in the timeline
    # dialog, addressed directly to the master) so an unsafe phrase
    # there is the worst place to leak it. master_phrase will surface
    # in future chat prompts so it's also a real-text channel — same
    # filter applies.
