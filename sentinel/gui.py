...
class HomeTab(QWidget):
    ...
    def __init__(self):
        ...
        # 裝備 card 已移除 — equipment 系統 archive，per ADR
        # 2026-04-30-slime-stays-private.md。v0.8 加身體個體化視覺
        # 訊號（per ADR 2026-05-01-slime-physical-individuation.md）
        # 之後可能在這個位置長出新的「形態 / 紋路」card。
    ...
    def refresh(self):
        ...
        # equip_card removed — equipment system archived
...
# ─── EquipmentTab + MarketTab REMOVED ────────────────────────────────────
# Per ADR docs/decisions/2026-04-30-slime-stays-private.md.
# Equipment is a showing-off mechanic; marketplace turns Slime into a
# platform; both violate「Slime stays private」. Class definitions and
# their inline imports of sentinel.wallet.equipment / equipment_visuals /
# wallet.market_rules removed here. Modules archived to
# archive/sentinel-side/. EVOLVE_COST + SPEND_TYPE_EVOLVE moved to
# sentinel/wallet/costs.py so server/evolution/router.py + EvolutionTab
# keep importing.
#
# Visual layer (overlay.py / slime_avatar.py / sprite_renderer.py) had
# its equipment-rendering branches stubbed out in the same PR — slime
# now renders as base form. v0.8 cycle adds birth_signature per ADR
...
class MemoryTab(QWidget):
    def __init__(self):
        ...
        # 箱子 — every memorable_moment in one place, the full thread
        # of accumulated time visible. ADR 共同沉積 mechanism 1
        # (「箱子要可以被主人翻」). Day-of-life prominent in each row
        # because timestamp is the asymmetric thing slime has —
        # capability commoditizes, time doesn't.
        box_group = QGroupBox("箱子 — 跟主人走過的時刻")
        box_layout = QVBoxLayout()
        self.box_text = QTextEdit()
        self.box_text.setReadOnly(True)
        # No fixed height — let it stretch with the layout, since this
        # is the section the user is meant to actually flip through.
        box_layout.addWidget(self.box_text)
        box_group.setLayout(box_layout)
        layout.addWidget(box_group, stretch=2)
    ...
    def refresh(self):
        ...
        # 箱子 — render every memorable_moment with day_n prominent.
        # Reads through identity.list_box_entries which enriches each
        # row with day_n + has_letter / has_phrase flags.
        from sentinel.identity import list_box_entries
        from sentinel.evolution import load_evolution
        try:
            _evo = load_evolution()
            _birth = float(getattr(_evo, "birth_time", 0) or 0)
        except Exception:
            _birth = 0
        box_entries = list_box_entries(birth_time=_birth, newest_first=True)
        self.box_text.setHtml(_render_box_html(box_entries))
...
class EvolutionTab(QWidget):
    ...
    def refresh(self):
        ...
        # set_equipment call removed — equipment system archived per
        # ADR 2026-04-30-slime-stays-private.md. Visual layering will
        # be re-introduced via birth_signature in v0.8 per ADR
        # 2026-05-01-slime-physical-individuation.md.
        ...
        if info["at_max"]:
            ...
            if mode == "byok":
                ...
                from sentinel.wallet.costs import EVOLVE_COST
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # equipment_tab / market_tab / federation_tab all archived per
        # ADR 2026-04-30-slime-stays-private.md. Their class definitions
        # are gone and modules moved to archive/. Slime visual layer
        # falls back to base form (overlay/avatar/sprite_renderer
        # stubbed) until v0.8 birth_signature work per ADR
        # 2026-05-01-slime-physical-individuation.md.
    ...
    def _start_daemon(self):
        def _run():
            ...
            # equipment imports removed — module archived per ADR
            # 2026-04-30-slime-stays-private.md. try_drop / exp_multiplier
            # call sites in this loop are removed too.
            ...
            while self.daemon_running:
              try:
                ...
                if now - last_check >= config.SYSTEM_CHECK_INTERVAL:
                    ...
                    # Equipment EXP buff removed — equipment archived
                    # per ADR 2026-04-30-slime-stays-private.md.
                    ...
                    # 裝備掉落 (try_drop) 移除 — equipment archived per
                    # ADR 2026-04-30-slime-stays-private.md. obs_since_drop
                    # counter and the per-100-obs drop check both retired.
                    ...
                    if now - last_distill >= config.DISTILL_INTERVAL and activity_buf:
                        ...
                        if result is not None:
                            ...
                            # 學習完成的裝備掉落 (try_drop) 移除 —
                            # equipment 系統 archive，per ADR
                            # 2026-04-30-slime-stays-private.md.
